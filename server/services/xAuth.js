...
console.log('X Auth Configuration:', {
...
export function getXLoginUrl(state) {
  ...
  // Use twitter-api-v2 library with proper scopes format
  ...
  console.log('Generated auth URL:', authLink.url);
  console.log('Scopes requested:', ['users.read', 'tweet.read', 'follows.read', 'offline.access']);
...
export async function handleXCallback(code, state) {
  ...
  // For PKCE OAuth 2.0, don't use client secret during token exchange
  const client = new TwitterApi({ 
    clientId
    // Note: client secret not used in PKCE flow
  });
